...
### Modify By Neo www.skc.la ###
mkdir -p /usr/local/directadmin
tar xvf DirectAdmin.tar.gz -C /usr/local/directadmin

echo "DA install scripts ready ...";
### Modify By Neo www.skc.la ###
...
if [ "$OS" = "FreeBSD" ]; then
	...
	if [ "$B64" -gt 0 ]; then
		echo "*** 64-bit OS ***";
		echo "*** 这样说，这应该是一个FreeBSD的7，8或9的系统。 ***";
		sleep 2;
		B64=1
	fi
else
...
if [ -e /usr/local/directadmin ]; then
	...
	echo "";
	echo "*** 管理向导已经存在 ***";
	echo "    接下来的10秒内按Ctrl-C取消安装";
	echo "    否则，等待，安装将继续，但会破坏现有的数据";
	echo "";
	echo "";
	sleep 10;
fi
...
if [ $OS = "FreeBSD" ]; then
	...
	if [ "$OS_VER" = "wheezy/sid" ]; then
		...
		echo "********************************************************************************************";
		echo "";
		echo "本脚本由VPSPS.COM编写.";
		echo "请安装debian6.x的，而不是";
		echo "";
		echo "参阅Debian发行版：";
...
if [ "$CMD_LINE" -eq 1 ]; then
    ...
    echo "*****************************************************";
    echo "*";
	echo "*   Directadmin v1.44.3 Nulled by VPSPS.COM";
	echo "*   Improved By VPSPS.COM";
	echo "*   VPSPS.COM";
	echo "*";
	echo "*****************************************************";
fi
...
do
{
	...
	echo -e "请输入您的主机名\（www.vpsps.com\）";
	echo "它必须是一个完全合格的域名";
	echo "*不*要使用你打算使用主机名域：";
	echo "例如。不要使用domain.com。使用server.domain.com代替。";
	echo "请勿输入http：//或www";
	echo "";
	echo -n "输入您的主机名（FQDN）： ";
	read HOST;
...
if [ $OS = "FreeBSD" ]; then
	...
	if [ $CMD_LINE -eq 0 ]; then
		...
		if [ $COUNT -eq 0 ]; then
        		echo "找不到您的以太网设备。";
	        	echo -n "请输入您的以太网设备的名称：";
	        	read ETH_DEV;
		elif [ $COUNT -eq 1 ]; then
        		echo -n "这个 $DEVS 是你服务器的IP? (y,n) : ";
		        read yesno;
        		if [ "$yesno" = "n" ]; then
                		echo -n "输入您想使用的以太网设备的名称： ";
		                read ETH_DEV;
		        else
	        	        ETH_DEV=$DEVS
		        fi
	...
	if [ $CMD_LINE -eq 0 ]; then
		...
		if [ $COUNT -eq 0 ]; then
			...
			DIP=`/sbin/ifconfig $DEVS | grep 'inet addr:' | cut -d: -f2 | cut -d\  -f1`;
        		...
        		echo -n "这个 $DEVS 是你服务器的IP? ($DIP)? (y,n) : ";
		        read yesno;
        		if [ "$yesno" = "n" ]; then
                		echo -n "输入您想使用的以太网设备的名称： ";
		                read ETH_DEV;
	        	...
	        	# more than one
		        echo "以下以太网设备/ IP地址被发现了。请输入您想使用的设备的名称：";
		        echo "";
		        #echo $DEVS;
		        for i in $DEVS; do
		        {
...
if [ $CMD_LINE -eq 0 ]; then
	...
	if [ "$IP" = "" ]; then
		...
		echo -n "这个 $IP 他在你的许可证？ (y,n) : ";
		read yesno;
	fi

	if [ "$yesno" = "n" ]; then
		echo -n "输入您的许可证文件中使用的IP： ";
		read IP;
	fi

	if [ "$IP" = "" ]; then
		echo "输入的IP是空白。请重试，并输入一个有效的IP";
	fi
fi
...
if [ $CMD_LINE -eq 0 ]; then
        echo "";
        echo "现在，您有4个选项，你的Apache/ PHP的安装程序（请选择“3”其他可能失败）。";
        echo "";
        echo "1: 自定义的Apache:         end-of-life software.  Apache 1.3, php 4, frontpage.  Do not use.  No support.";
        echo "2: 自定义生成1.1:      older software.  Apache 2.x, php 4, 5, or both in cli and/or suphp. Defaults to php 5.2";
	echo "3: 自定义生成1.2:      * Recommended. simlar to 1.1, newer default versions.  Apache 2.x, php 5, 6, or both in cli and/or suphp. Defaults to php 5.3";
	echo "4: 自定义生成2.0 ALPHA: Not yet recommended, still in testing. Apache 2.4, mod_ruid2, php 5.4. Can be set to use php-FPM. Experts only. Requires pre-release binaries.";
	echo "";
	echo "      custombuild问题论坛：http://www.directadmin.com/forum/forumdisplay.php?f=61";
        echo "";
	...
	echo -n "Enter your choice (1, 2, 3 or 4): ";
		...
		chmod 755 $BUILD
                ...
                echo "";
                echo -n "你想的Apache的默认设置 ${AP_VER}${RUID} 和 php ${PHP_V} ${PHP_T}? (y/n): ";
                read yesno;
                if [ "$yesno" = "n" ]; then
                        echo "你选择自定义自定义生成选项。请稍候选项配置器下载... ";
                        echo "";

                        if [ -e $BUILD ]; then
                                $BUILD create_options
                        else
                                echo "无法下载构建文件。使用默认值来代替。";
                        fi
                else
                        echo "使用默认设置，自定义生成。";
                fi
                
                echo -n "你想寻找最快的下载镜像？ (y/n): ";
                read yesno;
                if [ "$yesno" = "y" ]; then
                	$BUILD set_fastest;
			...
			if [ -s "${CB_OPTIONS}" ]; then
                ...
                echo "使用自定义在Apache 1.3和PHP4的apache.";
        fi
